// Wire Master Writer
// by Nicholas Zambetti <http://www.zambetti.com>

// Demonstrates use of the Wire library
// Writes data to an I2C/TWI slave device
// Refer to the "Wire Slave Receiver" example for use with this

// Created 29 March 2006

// This example code is in the public domain.


#include <Wire.h>

bool loopNum = 0;

void setup() {
  Wire.begin(); // join i2c bus (address optional for master)
  Wire.beginTransmission(0x27); // transmit to device #8
  Wire.write(0x00);        // sends five bytes
  Wire.write(0x00);              // sends one byte
  Wire.endTransmission();    // stop transmitting

  Wire.beginTransmission(0x26); // transmit to device #8
  Wire.write(0x00);        // sends five bytes
  Wire.write(0x00);              // sends one byte
  Wire.endTransmission();    // stop transmitting
}


void loop() {
  int del = 50;
  //  char *expNum[] = {"0x27", "0x26"};
  uint8_t indexVal;
  if (loopNum == 1) {
    indexVal = 0x26;
  }
  else {
    indexVal = 0x27;
  }
  Wire.beginTransmission(indexVal); // transmit to device #8
  Wire.write(0x09);        // sends five bytes
  Wire.write(0x80);              // sends one byte
  Wire.endTransmission();    // stop transmitting
  delay(del);

  Wire.beginTransmission(indexVal); // transmit to device #8
  Wire.write(0x09);        // sends five bytes
  Wire.write(0x40);              // sends one byte
  Wire.endTransmission();    // stop transmitting
  delay(del);

  Wire.beginTransmission(indexVal); // transmit to device #8
  Wire.write(0x09);        // sends five bytes
  Wire.write(0x20);              // sends one byte
  Wire.endTransmission();    // stop transmitting
  delay(del);

  Wire.beginTransmission(indexVal); // transmit to device #8
  Wire.write(0x09);        // sends five bytes
  Wire.write(0x10);              // sends one byte
  Wire.endTransmission();    // stop transmitting
  delay(del);

  Wire.beginTransmission(indexVal); // transmit to device #8
  Wire.write(0x09);        // sends five bytes
  Wire.write(0x08);              // sends one byte
  Wire.endTransmission();    // stop transmitting
  delay(del);

  Wire.beginTransmission(indexVal); // transmit to device #8
  Wire.write(0x09);        // sends five bytes
  Wire.write(0x04);              // sends one byte
  Wire.endTransmission();    // stop transmitting
  delay(del);

  Wire.beginTransmission(indexVal); // transmit to device #8
  Wire.write(0x09);        // sends five bytes
  Wire.write(0x02);              // sends one byte
  Wire.endTransmission();    // stop transmitting
  delay(del);

  Wire.beginTransmission(indexVal); // transmit to device #8
  Wire.write(0x09);        // sends five bytes
  Wire.write(0x01);              // sends one byte
  Wire.endTransmission();    // stop transmitting
  delay(del);

  Wire.beginTransmission(indexVal); // transmit to device #8
  Wire.write(0x09);        // sends five bytes
  Wire.write(0x00);              // sends one byte
  Wire.endTransmission();    // stop transmitting
  delay(del);

  loopNum = !loopNum;
}
